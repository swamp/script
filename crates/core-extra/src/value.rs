...
impl QuickSerialize for Rc<RefCell<dyn AnyRustType>> {
    fn quick_serialize(&self, octets: &mut [u8]) -> usize {
...
impl Value {
    ...
    ///
    #[must_use]
    pub fn downcast_rust<T: AnyRustType + 'static>(&self) -> Option<Rc<RefCell<Box<T>>>> {
        match self {
    ...
    #[must_use]
    pub fn downcast_hidden_rust<T: AnyRustType + 'static>(&self) -> Option<Rc<RefCell<Box<T>>>> {
        match self {
