...
pub fn quick_deserialize(resolved_type: &Type, buf: &[u8], depth: usize) -> (Value, usize) {
    let (val, octet_size) = match resolved_type {
        ...
        Type::Struct(struct_type_ref) => {
            let mut values = Vec::new();
            let mut offset = 0;
            for struct_field_type in struct_type_ref.anon_struct_type.defined_fields.values() {
        ...
        Type::TypeParameterName(..) => {
            panic!("can not serialize type references")
        }
        ...
        Type::External(_rust_type_ref) => {
            ...
            match rust_type_ref.number {
                SPARSE_ID_TYPE_ID => {
                    ...
                    (
                        {
                            let boxed_sparse_value_id: Rc<RefCell<Box<dyn AnyRustType>>> =
            ...
            /*
             if let Type::RustType(found_rust_type) = &**base_type {
                if found_rust_type.number == SPARSE_TYPE_ID {
                    let sparse_type_id_rust_type = Rc::new(RustType {
                        type_name: "SparseId".to_string(),
                        number: SPARSE_ID_TYPE_ID, // TODO: FIX hardcoded number
                    });
                    ...
                    let sparse_collection_rust_type = Rc::new(RustType {
